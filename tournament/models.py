...
class Team(models.Model):
    ...
    def save(self, *args, **kwargs):
        is_new_team = self._state.adding
...
class BotSubmission(models.Model):
    ...
    code_file = models.FileField(upload_to='bot_scripts/', help_text="The .py file for the bot.")
    submitted_at = models.DateTimeField(auto_now_add=True)
    is_active = models.BooleanField(default=False, help_text="Is this the bot currently active?")
...
class Match(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    played_at = models.DateTimeField(null=True, blank=True, help_text="Timestamp of when match was played.")
    ...
    is_player2_system_bot = models.BooleanField(default=False, help_text="True if player2 is the system bot.")
    ...
    round_stage = models.PositiveIntegerField(
        null=True,
        blank=True
    )
    ...
    def clean(self):
        ...
        if self.match_type in [self.MatchType.ROUND_ONE, self.MatchType.TEST_VS_SYSTEM]:
            ...
            if self.player1_submission.team == self.player2_submission.team:
                 raise ValidationError("A team cannot play against itself in matches.")
        else: 
            if self.is_player2_system_bot and self.player2_submission:
                raise ValidationError("Cannot have both player2_submission and is_player2_system_bot for a normal match.")

    def save(self, *args, **kwargs):
        if self.match_type in [self.MatchType.ROUND_ONE, self.MatchType.TEST_VS_SYSTEM]:
            self.is_player2_system_bot = True
            self.player2_submission = None 
        elif self.match_type in [self.MatchType.ROUND_TWO, self.MatchType.CHALLENGE]:
